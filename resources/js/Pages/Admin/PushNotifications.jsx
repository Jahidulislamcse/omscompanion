...
export default function PushNotifications({ onesignal_app_id, onesignal_rest_api_key, notifications = [] }) {
    ...
    const handleSaveSettings = (e) => {
        e.preventDefault();
        postSettings(route('admin.push_notifications.settings'));
    };
    ...
    return (
        <AdminLayout title="Push Notifications">
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
                ...
                {/* Tab Navigation */}
                <div style={{ display: 'flex', gap: '10px', marginBottom: '25px', borderBottom: '1px solid var(--border-color, #e2e8f0)', paddingBottom: '10px' }}>
                    <button
                    ...
                    <button
                        ...
                        style={{ padding: '10px 20px', borderRadius: '10px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}
                    >
                        <span>⚙️</span> OneSignal API Credentials
                    </button>
                </div>
                    ...
                    <div className="grid-2-cols" style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '25px' }}>
                        ...
                        <div className="glass-panel" style={{ padding: '25px', borderRadius: '16px' }}>
                            ...
                            {!onesignal_app_id || !onesignal_rest_api_key ? (
                                <div style={{ backgroundColor: '#fffbe6', border: '1px solid #ffe58f', padding: '15px', borderRadius: '10px', marginBottom: '20px', color: '#873800' }}>
                                    ⚠️ <strong>OneSignal Credentials Missing:</strong> Please set your App ID and REST API Key in the <strong>OneSignal API Credentials</strong> tab first.
                                </div>
                            ) : null}
                            ...
                            <form onSubmit={handleSendPush}>
                                ...
                                <button
                                    ...
                                    disabled={processingPush || !onesignal_app_id}
                ...
                {/* Tab 2: OneSignal API Credentials */}
                {activeTab === 'settings' && (
                    <div className="glass-panel" style={{ padding: '30px', borderRadius: '16px', maxWidth: '700px' }}>
                        <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <span>⚙️</span> OneSignal API Configuration
                        </h3>

                        <p style={{ color: 'var(--text-muted, #64748b)', fontSize: '14px', marginBottom: '25px' }}>
                            You can get your App ID and REST API Key from your <a href="https://onesignal.com" target="_blank" rel="noopener noreferrer" style={{ color: '#0d9488', textDecoration: 'underline' }}>OneSignal Dashboard</a> under <strong>Settings &gt; Keys &amp; IDs</strong>.
                        </p>

                        <form onSubmit={handleSaveSettings}>
                            <div className="form-group" style={{ marginBottom: '20px' }}>
                                <label className="form-label" style={{ fontWeight: '600', display: 'block', marginBottom: '6px' }}>
                                    OneSignal App ID <span style={{ color: 'var(--color-danger, #ef4444)' }}>*</span>
                                </label>
                                <input
                                    type="text"
                                    className="form-control"
                                    placeholder="e.g. 1a2b3c4d-5e6f-7a8b-9c0d-1e2f3a4b5c6d"
                                    value={settingsData.onesignal_app_id}
                                    onChange={(e) => setSettingsData('onesignal_app_id', e.target.value)}
                                    required
                                    style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color, #cbd5e1)', fontFamily: 'monospace' }}
                                />
                            </div>

                            <div className="form-group" style={{ marginBottom: '25px' }}>
                                <label className="form-label" style={{ fontWeight: '600', display: 'block', marginBottom: '6px' }}>
                                    OneSignal REST API Key <span style={{ color: 'var(--color-danger, #ef4444)' }}>*</span>
                                </label>
                                <input
                                    type="password"
                                    className="form-control"
                                    placeholder="e.g. os_v2_app_..."
                                    value={settingsData.onesignal_rest_api_key}
                                    onChange={(e) => setSettingsData('onesignal_rest_api_key', e.target.value)}
                                    required
                                    style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color, #cbd5e1)', fontFamily: 'monospace' }}
                                />
                            </div>
                            ...
                            <button
                                ...
                                style={{ padding: '12px 24px', borderRadius: '8px', fontWeight: '700' }}
                            >
                                {processingSettings ? 'Saving Settings...' : '💾 Save OneSignal Settings'}
                            </button>
                        </form>
                    </div>
                )}
